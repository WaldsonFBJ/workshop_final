import 'formatted_time_generator.dart';
  ...
  factory TvInfoModel.fromJson(json) {
    ...
    return TvInfoModel(
      ...
      languages: (json['spoken_languages'] as List)
          ...
          .toList(),
      created:
          (json['created_by'] as List).map((lang) => lang['name']).toList(),
      genres: (json['genres'] as List).map((lang) => lang['name']).toList(),
      networks:
          (json['networks'] as List).map((lang) => lang['name']).toList(),
      ...
      episodeRuntime: (json['episode_run_time'] as List).isNotEmpty
          ? json['episode_run_time'][0].toString() + " Minutos"
          : "N/A",
      formatedDate: string,
    );
  }
...
class Seasons {
  ...
  factory Seasons.fromJson(json) {
    ...
    getString() {
      try {
        string =
            "estreiou em ${monthgenrater(json['air_date'].split("-")[1])} ${json['air_date'].split("-")[2]}, ${json['air_date'].split("-")[0]}";
        // ignore: empty_catches
      } catch (e) {}
    }
